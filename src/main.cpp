/*
 * Main EXE — Scanner + injector with embedded hook DLL payloads.
 * Cross-arch injection (64->32) via PE export table walking.
 * Single output file: extracts DLLs from resources at runtime.
 */
#undef UNICODE
#undef _UNICODE
#include <windows.h>
...
#ifndef _WIN64
#pragma message("WARNING: 32-bit build cannot inject 64-bit targets.")
#endif
...
/* ---------- Cross-arch injection (64->32) via PE export walking ---------- */
...
static LPTHREAD_START_ROUTINE Resolve32LoadLibA(HANDLE hp, DWORD pid) {
    ...
    HeapFree(GetProcessHeap(), 0, nrva);
    HeapFree(GetProcessHeap(), 0, ords);
    HeapFree(GetProcessHeap(), 0, frva);
    return result; }
...
int main(void) {
    ...
    printf("=== CancelWindowDisplayAffinity (Hook Edition) ===\n");
#ifdef _WIN64
    printf("64-bit — supports both 32-bit and 64-bit targets\n\n");
#else
    printf("32-bit — supports 32-bit targets only\n\n");
#endif

    /* Extract DLL payloads to temp */
    char tmp[MAX_PATH]; GetTempPathA(MAX_PATH, tmp);
    char dll32[MAX_PATH], dll64[MAX_PATH];
    snprintf(dll32, MAX_PATH, "%sHookAffinity32.dll", tmp);
    snprintf(dll64, MAX_PATH, "%sHookAffinity64.dll", tmp);
    ...
    for (int i = 0; i < ctx.n; i++) {
...
#ifdef _WIN64
        if (t64) {
            printf("  Method: direct injection\n");
        ...
        if (!t64) {
            ...
            printf("  [SKIP] Cannot inject 64-bit target from 32-bit.\n");
        }
#endif
        printf("  Result: %s\n\n", ok ? "SUCCESS" : "FAILED");
    }
